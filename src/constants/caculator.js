export const executeResult = (data) => {
  ...
  var threshold = 10; // ngưỡng để xác định y
  ...
  for (var i in res) {
    if (i == 0) continue;
    var bounds = res[i].boundingPoly.vertices;
    var thisavg = (bounds[2].y + bounds[3].y) / 2;// Middle y of this word

    if (line < 0) line = thisavg;

    // check if the middle of the 'word' is within the line threshold
    if (Math.abs(thisavg - line) <= threshold) {
      sentence += " " + res[i].description; // within the threshold, add to line with 'space delimited'
    }
      ...
      sentence = res[i].description; // create new line
      startx += bounds[0].x;
      startxcount++;
    }
    line = thisavg;
